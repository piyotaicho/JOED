...
機能体験用のWebアプリは https://p4testsuite.hostingerapp.com/JOEDv5/Web/ でご利用いただけます.
...
データベースの削除.
allでバックアップも削除.
lockではロックファイルのみを削除する.

#### --datadir=ディレクトリ
データベースの保存ディレクトリを指定. このオプションを使用した場合はロックファイルを作成します.
...
|NumberOfCases              |integer| |入力症例数（キュメントが保持するの症例数はNumberOfCases - NumberOfDecline）|
|NumberOfDecline            |integer| |登録拒否症例数|
|Version                    |string | |提出データ作成時のソフトウエアのバージョン|
|Plathome                   |string | |使用環境調査 plathome (arch)|
|hash                       |string | |症例データ部分だけのハッシュ値|

### オブジェクト:提出データ
|名称                        | タイプ |フォーマット規則|必須項目|解説|
|:--------------------------|:-----:|:----------:|:--:|:---|
|JSOGId                     |string |^(CC|EM|US|UAS|OV|VU|VA|TD)20\d{2}-\d+$| |日産婦腫瘍登録番号(腫瘍委員会の臨床研究において使用)|
|ProcedureTime              |string | |X|手術時間表記テーブルから引用される|
|TypeOfProcedure            |string |(腹腔鏡\|腹腔鏡悪性\|ロボット支援下\|ロボット支援下悪性\|子宮鏡\|卵管鏡)|X|主たる術式の種別、Procedures配列の最上位の順位のものが採用される|
|PresentAE                  |boolean| |X|合併症の登録があればtrue =(AEs.length>0)|
|Diagnoses                  |array  | |X|診断オブジェクト - Diagnosis|
|Procedures                 |array  | |X|術式オブジェクト - Procedure(AdditionalProcedureはProcedureに投影して次のエレメントとして登録される)|
|AEs                        |array  | | |合併症オブジェクト - AE|
|hash                       |string | |X|レコードのハッシュ値|
...
categoryは ["腹腔鏡","ロボット支援下","子宮鏡","卵管鏡"] のいずれか.
targetは対象臓器 ["子宮","付属器","その他"] のいずれか.
```javascript
{
    '{category}': {
        '{target}': {
...
|ICD10                      |array  |対応するICD-10コード(*で前方一致)|
|Notification               |string |入力時に表示されるおしらせ(未実装)|
|Procedure                  |string |1:1で紐付けられた術式(未実装)|
|VaildFrom                  |integer|適用可能年開始|
|VaildTo                    |integer|適用可能年終了|

### オブジェクト:DiagnosisItems
categoryは ["腹腔鏡","腹腔鏡悪性","ロボット","ロボット悪性","子宮鏡","卵管鏡"] のいずれか.
targetは対象臓器 ["子宮","付属器","その他"] のいずれか.
```javascript
{
    'category': {
...
### マスタオブジェクト:ProcedureMaster
|名称                        |タイプ  |解説
|:--------------------------|:-----:|:--|
|Procedure                  |string |手技名|
|STEM7                      |string |STEM7コード(未実装)|
|Kcode                      |array  |対応する社保Kコード(Kxxx-0x-0x形式, *で前方一致)|
|VaildFrom                  |string |適用可能年開始|
|VaildTo                    |string |適用可能年終了|
...
|Descriptions               |array  |補助情報の候補<br/>$MULTI$をメンバにもつ場合複数の内容を保持できる.<br/>$で終わるメンバはDescription自体作成されない.|
